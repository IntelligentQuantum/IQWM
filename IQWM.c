...
#ifndef __OpenBSD__
static int getiqblockspid();
static void sigiqblocks(const Arg *arg);
#endif
...
#ifndef __OpenBSD__
void sigiqblocks(const Arg *arg)
{
	union sigval sv;
	sv.sival_int = 0 | (iqblockssig << 8) | arg->i;
	if (!iqblockspid)
		if (getiqblockspid() == -1)
			return;

	if (sigqueue(iqblockspid, SIGUSR1, sv) == -1)
	{
		if (errno == ESRCH)
		{
			if (!getiqblockspid())
				sigqueue(iqblockspid, SIGUSR1, sv);
		}
	}
}
#endif
